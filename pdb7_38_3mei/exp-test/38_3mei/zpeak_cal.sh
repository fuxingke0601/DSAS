...
#echo ${filetitle}
 ...
 fft HKLIN ${bestCC_mtz} MAPOUT ${filetitle}_bestCC_map.map >${filetitle}_peak_cal_log.txt<<END
title fft_to_map
xyzlim asu
scale F1 1.0
...
END
 ...
 peakmax MAPIN ${filetitle}_bestCC_map.map XYZOUT ${filetitle}_pdb_CF.pdb XYZFRC ${filetitle}_peaks_ha.txt >>${filetitle}_peak_cal_log.txt<<END
threshhold rms 1.2
numpeaks $num
...
awk '{if(NR>2) max=($6>max)?$6:max} END{OFS="\t";}{if(NR>2) $6=$6/max;if(NR>2) print $6}' ${filetitle}_peaks_ha.txt | awk -v atom="${atom_type}" 'NR==FNR{a[NR]=$1; nr=NR} NR>FNR{if (FNR<=4) print $0;
                           else if (NF>1) printf "%s %6d %3s %4s %s %3d %11.3f %7.3f %7.3f %5.2f %5.2f %11s\n","ATOM",$6,$3,$4,$5,$6,$7,$8,$9,a[FNR-4],20,atom;
                           else print $0}' - ${filetitle}_pdb_CF.pdb >${filetitle}_pdb_CF_format.pdb
...
if [ ${flag} == 1 ];then
   ...
   if echo $column_type | grep -q ${letter1} && echo $column_type | grep -q ${letter2} ; then
       echo "mtz file have F+,F-" >${filetitle}_ctruncate.log
   else
       ctruncate -mtzin ${mtz_file} -mtzout "ctruncate.mtz"  -colano "/*/*/[I(+),SIGI(+),I(-),SIGI(-)]" >${filetitle}_ctruncate.log
 ...
 bp3 HKLIN ${mtz_file} HKLOUT ${filetitle}_Bp3.mtz << END
TARG SAD
XTAL bp3_phase
DNAM bp3_phase
COLU F+=F(+) SF+=SIGF(+) F-=F(-) SF-=SIGF(-)
FORM SE FP=${fprime1} FPP=${fprime2}
MODL ${filetitle}_pdb_CF.pdb
...
grep "HETATM" heavy-1.pdb |awk 'NR==FNR{a[NR]=$6; b[NR]=$7; c[NR]=$8; o[NR]=$9; bb[NR]=$10; nr=NR} NR>FNR{if (FNR<=4) print $0;
                           ...
                           else print $0}' - ${filetitle}_pdb_CF_format.pdb >${filetitle}_heavy-1_format.pdb

grep "HETATM" heavy-oh-1.pdb |awk 'NR==FNR{a[NR]=$6; b[NR]=$7; c[NR]=$8; o[NR]=$9; bb[NR]=$10; nr=NR} NR>FNR{if (FNR<=4) print $0;
                           else if (NF>1) printf "%s %6d %3s %4s %s %3d %11.3f %7.3f %7.3f %5.2f %5.2f %11s\n","ATOM",$2,$3,$4,$5,$6,a[FNR-4],b[FNR-4],c[FNR-4],o[FNR-4],bb[FNR-4],$12;
                           else print $0}' - ${filetitle}_pdb_CF_format.pdb >${filetitle}_heavy-oh-1_format.pdb
